...
void backgroundSceneDraw(Model &disc, Vector3 &warpSource, Vector3 &discAxis, float discXup, float &discRotation, float &discSize) {
    DrawModelEx(disc, warpSource, discAxis, discRotation, Vector3{discSize*discXup, discSize, discSize}, WHITE);
}
...
int main(void) {
    ...
    Shader posShader = LoadShader("shaders/vertex_info.vs", "shaders/pos_encode.fs");

    Camera cam = Camera{0};
    
    //warp buffer, and position buffer
    RenderTexture2D texBuffer = LoadRenderTexture(win_x, win_y);
    RenderTexture2D posBuffer = LoadRenderTexture(win_x, win_y);

    // shader to write world position into a color render target
    int shd_positionTex = GetShaderLocation(warpShader, "positionTex");
    ...
    int posshd_camPos = GetShaderLocation(posShader, "camPos");
    int posshd_warpSource = GetShaderLocation(posShader, "warpSource");
    float warpSourceArr[3] = {warpSource.x, warpSource.y, warpSource.z};
    SetShaderValue(posShader, posshd_warpSource, &warpSourceArr, SHADER_UNIFORM_VEC3);
    
    int posshd_warpRadius = GetShaderLocation(posShader, "sourceSize");
    SetShaderValue(posShader, posshd_warpRadius, &sourceSize, SHADER_UNIFORM_FLOAT);
    ...
    while (!WindowShouldClose()) {
        ...
        discRotation += 45.f * GetFrameTime();
        ...
        SetShaderValue(posShader, posshd_camPos, &camPos, SHADER_UNIFORM_VEC3);
        ...
        // render scene positions into posBuffer using posShader
        BeginTextureMode(posBuffer);
            ClearBackground(BLACK);
            BeginMode3D(cam);
                BeginShaderMode(posShader);
                    backgroundSceneDraw(fakeDisc, warpSource, discAxis, 1.2f, discRotation, discSize);
                EndShaderMode();
            EndMode3D();
        EndTextureMode();

        // render scene w/ warpshader into texBuffer
        BeginTextureMode(texBuffer);
            ...
            BeginMode3D(cam);
                ...
                backgroundSceneDraw(realDisk, warpSource, discAxis, 1.2f, discRotation, discSize);
            EndMode3D();
        EndTextureMode();
        ...
        BeginDrawing();
            ...
            BeginShaderMode(warpShader);
                SetShaderValueTexture(warpShader, shd_positionTex, posBuffer.texture);
                DrawTextureRec(texBuffer.texture, Rectangle{0, 0, (float)win_x, (float)-win_y}, Vector2{0, 0}, WHITE);
            EndShaderMode(); //*/

            //DrawTextureRec(posBuffer.texture, Rectangle{0, 0, (float)win_x, (float)-win_y}, Vector2{0, 0}, WHITE);

            BeginMode3D(cam);
                //Color discColor = WHITE;
                //discColor.a = (unsigned char)(discAlpha * 240);
                //DrawModelEx(realDisk, warpSource, discAxis, discRotation, Vector3{discSize, discSize, discSize}, discColor);
                
                //DrawSphere(warpSource, sourceSize, BLUE);
                //DrawSphereWires(warpSource, sourceSize + 0.05f, 10, 10, DARKBLUE);
                
                //3D axis lines for reference
